...
const SPONSORED_PRODUCTS: SponsoredProduct[] = [
  {
    ...
    url: "https://www.filippoberio.co.uk/",
  },
  {
    id: "sp2",
    brand: "Yeo Valley",
...
async function loadRecipesFromSupabase(): Promise<Recipe[] | null> {
  ...
  const { data, error } = await supabase
    .from("recipes")
    .select(
      "id, title, description, meal_type, image_url, prep_time, cook_time, serves, cuisine, tags, allergens, ingredients, method, note, approved_for_planning, created_by, source_type"
    )
  ...
  return data.map((row: any) => ({
    ...
    serves: row.serves ?? 4,
    cuisine: normaliseCuisineLabel(row.cuisine ?? "British", row.title ?? "", row.description ?? ""),
    tags: row.tags ?? [],
...
async function ensureHousehold(userId: string): Promise<{ id: string; name: string } | null> {
  if (!supabase) return null;
  ...
  if (existing && existing.length > 0) return { id: existing[0].id, name: existing[0].name || "My household" };
  ...
  return { id: inserted[0].id, name: inserted[0].name || "My household" };
}


async function updateHouseholdNameInSupabase(householdId: string, name: string): Promise<void> {
  if (!supabase) return;
  const { error } = await supabase.from("households").update({ name }).eq("id", householdId);
  if (error) console.error("updateHouseholdNameInSupabase error:", error);
}
...
async function updateRecipeInSupabase(recipe: Recipe): Promise<void> {
  if (!supabase) return;
  ...
  if (error) console.error("updateRecipeInSupabase error:", error);
}

async function deleteRecipeFromSupabase(recipeId: string): Promise<void> {
  if (!supabase) return;
  const { error } = await supabase.from("recipes").delete().eq("id", recipeId);
  if (error) console.error("deleteRecipeFromSupabase error:", error);
}
...
function normaliseCuisineLabel(cuisine: string, title = "", description = ""): string {
  const combined = `${cuisine} ${title} ${description}`.toLowerCase();

  if (combined.includes("mediterranean")) return "Mediterranean";
  if (combined.includes("italian")) return "Italian";
  if (combined.includes("french")) return "French";
  if (combined.includes("japanese")) return "Japanese";
  if (combined.includes("indian")) return "Indian";
  if (combined.includes("mexican")) return "Mexican";
  if (combined.includes("chinese")) return "Chinese";
  if (combined.includes("thai")) return "Thai";
  if (combined.includes("american")) return "American";
  if (combined.includes("british")) return "British";

  return cuisine || "British";
}
...
function isUserGeneratedRecipe(recipe: Recipe, userId: string | null): boolean {
  if (!userId) return false;
  return recipe.createdBy === userId || recipe.sourceType === "manual";
}

function makeRecipeFormFromRecipe(recipe: Recipe): RecipeForm {
  return {
    title: recipe.title,
    description: recipe.description,
...
function replaceRecipeInPlanner(planner: Planner, oldRecipeId: string, replacementRecipeId: string): Planner {
  const nextPlanner: Planner = JSON.parse(JSON.stringify(planner));
  for (const day of DAYS) {
    for (const mealType of MEAL_TYPES) {
      if (nextPlanner[day][mealType] === oldRecipeId) {
        nextPlanner[day][mealType] = replacementRecipeId;
      }
    }
  }
  return nextPlanner;
...
function buildCheckedShoppingItems(groups: ShoppingGroup[], selections: ShoppingSelectionMap): string[] {
  const lines: string[] = [];

  groups.forEach((group) => {
    const selectedItems = group.items.filter((item) => selections[makeShoppingSelectionKey(item)]);
    const selectedSponsored =
      group.sponsoredProduct && selections[makeSponsoredSelectionKey(group.sponsoredProduct.id)]
        ? [`${group.sponsoredProduct.brand} — ${group.sponsoredProduct.product} (Sponsored pick)`]
        : [];

    const combined = [...selectedItems, ...selectedSponsored];
    if (combined.length === 0) return;

    lines.push(group.category);
    combined.forEach((item) => lines.push(`• ${item}`));
    lines.push("");
  });
...
export default function App() {
  ...
  useEffect(() => {
    ...
    async function boot() {
      ...
      const ensuredHousehold = await ensureHousehold(userId);
      if (!isMounted) return;
      setHouseholdId(ensuredHousehold?.id ?? null);
      setHouseholdName(ensuredHousehold?.name ?? "My household");
      ...
      const [
        ...
        ensuredHousehold?.id ? loadMembersFromSupabase(ensuredHousehold.id) : Promise.resolve(null),
        ensuredHousehold?.id ? loadPlannerFromSupabase(ensuredHousehold.id, weekStart) : Promise.resolve(null),
        loadBillingProfileFromSupabase(userId),
        loadSubscriptionFromSupabase(userId),
        ensuredHousehold?.id ? loadShoppingSelectionsFromSupabase(ensuredHousehold.id, weekStart) : Promise.resolve({}),
      ]);
  ...
  useEffect(() => {
    if (!householdId || !supabase || isLoadingData || !userId || !householdName.trim()) return;

    const timeout = window.setTimeout(async () => {
      await updateHouseholdNameInSupabase(householdId, householdName.trim());
    }, 500);

    return () => window.clearTimeout(timeout);
  }, [householdId, householdName, isLoadingData, userId]);
  ...
  const householdFitLabel = useMemo(() => {
    const memberNames = members.map((member) => member.name).filter(Boolean);
    if (memberNames.length === 0) return householdName.trim() || "My household";
    return `${householdName.trim() || "My household"} — ${memberNames.join(", ")}`;
  }, [householdName, members]);

  const checkedShoppingLines = useMemo(
    () => buildCheckedShoppingItems(shoppingGroups, shoppingSelections),
    [shoppingGroups, shoppingSelections]
  );
  ...
  const exportCheckedShoppingItems = () => {
    if (checkedShoppingLines.length === 0) return;

    const content = `MealMagic checked shopping list\n${weekDateRange}\n\n${checkedShoppingLines.join("\n")}`;
    const blob = new Blob([content], { type: "text/plain;charset=utf-8" });
    const url = URL.createObjectURL(blob);
    const anchor = document.createElement("a");
    anchor.href = url;
    anchor.download = `mealmagic-shopping-list-${weekStart}.txt`;
  ...
  const printCheckedShoppingItems = () => {
    if (checkedShoppingLines.length === 0) return;
    ...
    popup.document.write(`
      <html>
        <head>
          <title>MealMagic Shopping List</title>
          <style>
            body { font-family: Arial, sans-serif; padding: 24px; color: #171717; }
            h1 { margin-bottom: 4px; }
            p { color: #525252; }
            pre { white-space: pre-wrap; font: inherit; line-height: 1.7; }
          </style>
        </head>
        <body>
          <h1>MealMagic checked shopping list</h1>
          <p>${weekDateRange}</p>
          <pre>${checkedShoppingLines.join("\n")}</pre>
        </body>
      </html>
    `);
  ...
  const startEditRecipe = (recipe: Recipe) => {
    ...
    setRecipeForm(makeRecipeFormFromRecipe(recipe));
  ...
  const saveRecipe = async () => {
    ...
    try {
      let finalImageUrl = editingRecipeId ? recipeMap[editingRecipeId]?.image || FALLBACK_RECIPE_IMAGE : FALLBACK_RECIPE_IMAGE;
      ...
      if (editingRecipeId) {
        const existingRecipe = recipeMap[editingRecipeId];
        ...
        const updatedRecipe: Recipe = {
          ...
          mealType: recipeForm.mealType,
          image: finalImageUrl,
          cuisine: normaliseCuisineLabel(
            recipeForm.cuisine.trim() || "British",
            recipeForm.title.trim(),
        ...
        setRecipes((prev) => prev.map((recipe) => (recipe.id === editingRecipeId ? updatedRecipe : recipe)));
  ...
  const deleteRecipe = async (recipe: Recipe) => {
    if (!canUseApp || !isUserGeneratedRecipe(recipe, userId)) return;

    const replacement = recipes.find(
      (candidate) => candidate.id !== recipe.id && candidate.mealType === recipe.mealType
    );
    ...
    if (replacement) {
      setPlanner((prev) => replaceRecipeInPlanner(prev, recipe.id, replacement.id));
    }
        ...
        setRecipes((prev) => {
          ...
          for (const recipe of data.generatedRecipes) {
            if (!merged.some((existing) => existing.id === recipe.id)) {
              merged.unshift({ ...recipe, cuisine: normaliseCuisineLabel(recipe.cuisine, recipe.title, recipe.description) });
            }
          }
          return merged;
        });
    ...
    return (
      <div style={{ minHeight: "100vh", display: "grid", placeItems: "center", padding: 24 }}>
        <div style={{ maxWidth: 420, textAlign: "center" }}>
          ...
          <p>Please log in to use MealMagic.</p>
          <button
            onClick={() => {
              window.location.href = "/login";
            }}
  ...
  <div
    style={{
      ...
      height: 48,
      borderRadius: 16,
      background: "linear-gradient(135deg,#f97316 0%, #fb923c 45%, #10b981 100%)",
      display: "flex",
      alignItems: "center",
      justifyContent: "center",
      boxShadow: "0 8px 22px rgba(16,185,129,0.16)",
      position: "relative",
      overflow: "hidden",
      flexShrink: 0,
    }}
  ...
  <div style={{ minWidth: 0 }}>
    <div style={{ fontSize: 13, color: "#737373", letterSpacing: 0.2 }}>
      Household meal planning
    </div>
    <div style={{ fontSize: 22, fontWeight: 800, lineHeight: 1.02 }}>
      <span style={{ color: "#171717" }}>Meal</span>
      <span style={{ color: "#f97316" }}>Magic</span>
    </div>
  </div>
</div>
      ...
      <main style={{ maxWidth: 1200, margin: "0 auto", padding: 16 }}>
        ...
        >
          ...
          <div style={cardStyle()}>
            ...
            <p style={{ color: "#737373" }}>{householdFitLabel}</p>
        ...
        >
          {(["planner", "household", "shopping", "manual", "recipes"] as const).map((key) => (
            ...
            >
              {key === "manual" ? (editingRecipeId ? "Edit recipe" : "Add recipe") : key.charAt(0).toUpperCase() + key.slice(1)}
            </button>
          ))}
        </div>
                ...
                <div style={{ minWidth: 220, width: "min(100%, 320px)" }}>
                  ...
                  {editingRecipeId && (
                    <button
                      ...
                      type="button"
                      style={{ borderRadius: 12, border: "1px solid #d4d4d4", background: "white", padding: "10px 14px", cursor: "pointer" }}
                    >
                      Cancel edit
                    </button>
                  )}
                            ...
                            <div style={{ display: "flex", gap: 12, marginBottom: 12 }}>
                              ...
                              <div style={{ minWidth: 0, flex: 1 }}>
                                <div style={{ display: "flex", gap: 8, alignItems: "center", marginBottom: 6, flexWrap: "wrap" }}>
                                  ...
                                  <span style={badgeStyle("#f5f5f5", "#404040")}>{normaliseCuisineLabel(recipe.cuisine, recipe.title, recipe.description)}</span>
...
<div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                              ...
                              {isUserGeneratedRecipe(recipe, userId) && (
                                <>
                                  <button
                                    onClick={() => startEditRecipe(recipe)}
                                    style={{ borderRadius: 12, border: "1px solid #d4d4d4", background: "white", padding: "8px 12px", cursor: "pointer" }}
                                  >
                                    <Pencil size={14} style={{ marginRight: 4, verticalAlign: "middle" }} /> Edit
                                  </button>
                                  <button
                                    onClick={() => void deleteRecipe(recipe)}
                                    style={{ borderRadius: 12, border: "1px solid #fecaca", background: "white", color: "#dc2626", padding: "8px 12px", cursor: "pointer" }}
                                  >
            ...
            >
              ...
              <div style={cardStyle()}>
                ...
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 12 }}>
                  <button
                    onClick={exportCheckedShoppingItems}
                    disabled={checkedShoppingLines.length === 0}
                    style={{ borderRadius: 12, border: "1px solid #d4d4d4", background: "white", padding: "8px 12px", cursor: checkedShoppingLines.length > 0 ? "pointer" : "not-allowed", opacity: checkedShoppingLines.length > 0 ? 1 : 0.6 }}
                  >
                    Export checked items
                  </button>
                  <button
                    onClick={printCheckedShoppingItems}
                    disabled={checkedShoppingLines.length === 0}
                    style={{ borderRadius: 12, border: "1px solid #d4d4d4", background: "white", padding: "8px 12px", cursor: checkedShoppingLines.length > 0 ? "pointer" : "not-allowed", opacity: checkedShoppingLines.length > 0 ? 1 : 0.6 }}
                  >
                    Print checked items
                          ...
                          >
                            ...
                            <button
                              ...
                              onClick={(e) => {
                                e.preventDefault();
                                e.stopPropagation();
                                openProductUrl(group.sponsoredProduct?.url);
                              }}
                              style={{ marginTop: 10, borderRadius: 12, border: "1px solid #bae6fd", background: "white", color: "#0369a1", padding: "8px 12px", cursor: "pointer" }}
                            >
                ...
                <div style={{ display: "grid", gap: 12 }}>
                  {SPONSORED_PRODUCTS.slice(0, 2).map((product) => (
                    <div key={product.id} style={{ border: "1px solid #dbeafe", background: "#f0f9ff", borderRadius: 16, padding: 12 }}>
                      ...
                      >
                        <ExternalLink size={14} style={{ marginRight: 6, verticalAlign: "middle" }} />View product
                      </button>
                    </div>
                  ))}
                </div>
              ...
              <div style={cardStyle()}>
                ...
                <p style={{ color: "#737373" }}>Simple referral link</p>
        ...
        {tab === "household" && (
          ...
          >
            <div style={cardStyle()}>
              ...
              <p style={{ color: "#737373", marginTop: -4, marginBottom: 16 }}>Add a household name and manage who the plan should fit.</p>
              ...
              <input
                ...
                style={{ width: "100%", padding: 12, borderRadius: 12, border: "1px solid #d4d4d4", marginBottom: 20 }}
              />
              ...
              <div style={{ display: "grid", gap: 12 }}>
                ...
                {selectedRecipe && isUserGeneratedRecipe(selectedRecipe, userId) && (
                  <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                    <button
                      onClick={() => startEditRecipe(selectedRecipe)}
                      style={{ borderRadius: 12, border: "1px solid #d4d4d4", background: "white", padding: "10px 12px", cursor: "pointer" }}
                    >
                    ...
                    <button
                      ...
                      style={{ borderRadius: 12, border: "1px solid #fecaca", background: "white", color: "#dc2626", padding: "10px 12px", cursor: "pointer" }}
                    >
            ...
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 16 }}>
              <button
                ...
                style={{
                  ...
                  background: "white",
                  padding: "10px 14px",
                  cursor: checkedShoppingLines.length > 0 ? "pointer" : "not-allowed",
                  opacity: checkedShoppingLines.length > 0 ? 1 : 0.6,
                }}
              >
                Export checked items
              </button>
        ...
        {tab === "manual" && (
          ...
          >
            <h3 style={{ marginTop: 0 }}>{editingRecipeId ? "Edit recipe" : "Add a recipe"}</h3>
            <p style={{ color: "#737373", marginTop: -4, marginBottom: 16 }}>
{editingRecipeId ? "Update your recipe and keep it in your household library." : "Create your own recipe and add it to your household library."}
            </p>
            ...
            <div style={{ display: "grid", gap: 16 }}>
              ...
              >
                ...
                {isSavingRecipe ? "Saving…" : editingRecipeId ? "Save recipe changes" : "Save recipe"}
              </button>
            </div>
          </div>
        )}
                ...
                <div style={{ paddingTop: 16 }}>
                  ...
                  {isUserGeneratedRecipe(recipe, userId) && (
                    <div style={{ display: "flex", gap: 8, marginTop: 10 }}>
                      <button onClick={() => startEditRecipe(recipe)} style={{ flex: 1, borderRadius: 12, border: "1px solid #d4d4d4", background: "white", padding: "10px 12px", cursor: "pointer" }}>Edit</button>
                      <button onClick={() => void deleteRecipe(recipe)} style={{ flex: 1, borderRadius: 12, border: "1px solid #fecaca", background: "white", color: "#dc2626", padding: "10px 12px", cursor: "pointer" }}>Delete</button>
                    </div>
                  )}
                </div>
              </div>
            ...
            >
              <div style={{ minWidth: 0, flex: 1 }}>
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 10 }}>
                  ...
                  <span style={badgeStyle("#f5f5f5", "#404040")}>{normaliseCuisineLabel(selectedRecipe.cuisine, selectedRecipe.title, selectedRecipe.description)}</span>
                  {freshRecipeIds.includes(selectedRecipe.id) && <span style={badgeStyle("#dcfce7", "#166534")}>New</span>}
                  {ratings[selectedRecipe.id] === "like" && <span style={badgeStyle("#f5f5f5", "#404040")}>Liked</span>}
                  {ratings[selectedRecipe.id] === "dislike" && <span style={badgeStyle("#dc2626", "white")}>Disliked</span>}
            ...
            >
              <div style={{ display: "grid", gap: 16, alignContent: "start" }}>
                ...
                <div style={{ border: "1px solid #e5e5e5", borderRadius: 16, padding: 16 }}>
                  ...
                  <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                    {ratings[selectedRecipe.id] === "like" ? (
                      <span style={badgeStyle("#dcfce7", "#166534")}>Liked by your household</span>
                    ) : ratings[selectedRecipe.id] === "dislike" ? (
                      <span style={badgeStyle("#fee2e2", "#b91c1c")}>Disliked by your household</span>
                    ) : (
                      <span style={badgeStyle("#f5f5f5", "#404040")}>No preference marked yet</span>
                    )}
                  </div>
                </div>
